...
/*
*esb:define start time and end time
*mongodb_conn_str and mongodb_conn_port
*mongodb_enable
*/
...
/*esb:define mongodb function*/
static void ngx_wirte_log_into_mongodb(ngx_http_request_t *r, 
	ngx_http_proxy_loc_conf_t *plcf,int timeuse);
/*esb:conf function point--*/
/*esb:mongodb connection string config*/
static char * 
	ngx_http_mongodb_set_conn(ngx_conf_t *cf,ngx_command_t *cmd, void *conf);
/*esb:customize config in location*/
static char *
	ngx_http_proxy_pass_customize(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
/*esb:esb_requestlog_counts */
static char *
ngx_http_request_set_counts(ngx_conf_t *cf,ngx_command_t *cmd, void *conf);
/*-----*/

/*esb:define read request body*/
static u_char *
	ngx_http_proxy_read_request_body(ngx_http_request_t *r);
/*esb:define read response body*/
static u_char *
	ngx_http_proxy_read_response_body(ngx_http_request_t *r);

/*esb:define buffer append string*/
/*
static ngx_buf_t *
...
static void
    ngx_http_proxy_write_client_invoke_record(ngx_http_request_t *r,int timeuse);
...
static ngx_command_t  ngx_http_proxy_commands[] = {
    ...
    /*esb:esb_request_log request*/
    { ngx_string("esb_requestlog_counts"),
        NGX_HTTP_MAIN_CONF|NGX_HTTP_SRV_CONF|NGX_HTTP_LOC_CONF|NGX_HTTP_LIF_CONF
        |NGX_HTTP_LMT_CONF|NGX_CONF_TAKE12,
        ngx_http_request_set_counts,
        NGX_HTTP_LOC_CONF_OFFSET,
        0,
        NULL },    
	...
	ngx_http_proxy_loc_conf_t *plcf,int timeuse)
{
  ...
  conn_str = (char *)mongodb_conn_str.data;
  port = (int)mongodb_conn_port;


  if((timeuse >= plcf->invoke_max_time
	  &&	0 < plcf->invoke_max_time)
	  ...
	  && r->headers_out.status <= NGX_HTTP_INSUFFICIENT_STORAGE)) //timeusr over invoke_max_time or request error
  {
	  ...
	  if( status == MONGO_OK ) {
		 ...
		 bson_append_string( b, "location", (char *)plcf->location.data);
		 bson_append_string( b, "url",  (char *)plcf->url.data);
		 bson_append_string( b, "clientip",  (char *)r->connection->addr_text.data);

		 if( r->headers_in.x_forwarded_for == NULL )
		 {
			bson_append_string( b, "realclientip" , (char *)r->connection->addr_text.data);
		 }
		 else
		 {
			bson_append_string( b, "realclientip" , (char *)r->headers_in.x_forwarded_for->value.data);
		 }
		 ...
		 if(plcf->app_name.data != NULL)
		 {
			bson_append_string( b,"appname",(char *)plcf->app_name.data);
		 }
		 else
		 {
		 ...
		 bson_finish( b );
		 mongo_insert( conn,"vesb.request", b , NULL );
...
ngx_http_proxy_read_response_body(ngx_http_request_t *r)
{
	...
	{
		if (out->buf != NULL) { 
			response_content = ngx_palloc(r->pool, out->buf->last-out->buf->pos + 1);
			ngx_cpystrn(response_content, out->buf->pos, out->buf->last-out->buf->pos);
		}		
	    out = out->next;
	}
...
ngx_http_mongodb_set_conn(ngx_conf_t *cf,ngx_command_t *cmd, void *conf)
{
	...
	mongodb_conn_str = value[1];

	mongodb_conn_port = ngx_atoi(value[2].data, value[2].len);

	if (mongodb_conn_port == (ngx_int_t) NGX_ERROR) {
...
ngx_http_proxy_pass_customize(ngx_conf_t *cf,ngx_command_t *cmd, void *conf)
{
	...
	plcf->app_name = value[1];

	plcf->invoke_max_time = ngx_atoi(value[2].data, value[2].len);


	if (plcf->invoke_max_time == (ngx_int_t) NGX_ERROR) {
		 ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "invalid number proxy_pass_customize");
...
{
    ngx_str_t                  *value;
	ngx_int_t                  request_count,proxy_count;                     
    
	value = cf->args->elts;
    

	request_count = ngx_atoi(value[1].data, value[1].len);
	proxy_count =  ngx_atoi(value[2].data, value[2].len);

	
	if (request_count == (ngx_int_t) NGX_ERROR) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "invalid number \"%V\" for esb_requestlog_counts", &value[1]);

        return NGX_CONF_ERROR;
    }

	/*esb: ngx_stat_request*/
	(void) ngx_atomic_fetch_add(ngx_stat_requests, request_count);
    
	if (proxy_count == (ngx_int_t) NGX_ERROR) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "invalid number \"%V\" for esb_requestlog_counts", &value[2]);

        return NGX_CONF_ERROR;
    }

    /*esb: ngx_stat_proxy_request*/
	(void) ngx_atomic_fetch_add(ngx_stat_proxy_request, proxy_count);
...
ngx_http_proxy_finalize_request(ngx_http_request_t *r, ngx_int_t rc)
{
	...
	/*esb:write into mongodb*/
	if(mongodb_enable > 0)
	{
		ngx_wirte_log_into_mongodb(r,plcf,timeuse);
	}
    ...
    ngx_http_proxy_write_client_invoke_record(r, timeuse);
...
ngx_http_proxy_write_client_invoke_record(ngx_http_request_t *r,int timeuse)
{
    ...
    server_invoke_info = ngx_find_ngx_esb_kv_node(ngx_server_invoke_info,plcf->app_name);
